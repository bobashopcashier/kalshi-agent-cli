...
import { ALLOWED_COMMANDS, buildKalshiArgs, type KalshiQueryInput } from "./commands.js";
...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    tool({
      ...
      label: "Kalshi Query",
      description:
        "Run an allowlisted, non-mutating Kalshi CLI command with bounded JSON output. Use commands.list and commands.describe for offline discovery. Returns the CLI's versioned envelope, including UPSTREAM_SCHEMA_MISMATCH field paths.",
      optional: true,
      parameters: Type.Object(
        {
          command: commandSchema,
          targetCommand: Type.Optional(
            Type.String({
      ...
      async execute(params, config, context) {
        ...
        const input = params as KalshiQueryInput;
