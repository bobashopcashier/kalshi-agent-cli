...
- `--fields` is a local response projection. The compiled registry exposes and pre-validates `x-projectable-fields`, so invalid network-command selectors fail before execution while valid absent optional values are materialized as `null`. Projection never changes the upstream request, pagination cursor, local reconciliation filtering, or write plan digest. Collection projections retain only the projected items plus the upstream cursor.
...
All output uses the `kalshi.agent/v1` envelope and a per-command `output_contract_version` such as `kalshi.output/markets.list/v1`. Each registry command exposes that version alongside its response schema, top-level properties, projection paths, unconditional `x-required-fields`, and `x-required-field-types`. Successful HTTP responses are checked before projection and sanitization; missing, null, or wrong-type required paths and declared top-level type changes fail atomically as `UPSTREAM_SCHEMA_MISMATCH` with structured `missing_fields` or `type_mismatches` details. Additional optional fields are tolerated. Removal, rename, type, requiredness, or structural changes bump only the affected command contract; additive optional fields do not.
...
- Projectable response paths cover the supported read surface, and required identity paths protect collection items from silent structural drift. The compact response schemas are still not a lossless local copy of every upstream OpenAPI type constraint.
